...
class ExportDeletedSessionRecoveryZipUsecase {
  ...
  Future<DeletedSessionRecoveryZipResult> execute({
    ...
    try {
      ...
      final readme = '''
${AppInfo.appName} — Recovery export (deleted session)
...
This bundle is NOT intended for standard operational re-import or rating
sheet handoff. Identifiers such as plot_pk and assessment_id may need to be
...
''';
      ...
      final zipPath =
          '${dir.path}/${_recoveryZipNamePrefix()}_recovery_deleted_session_${sessionId}_${DateTime.now().millisecondsSinceEpoch}.zip';
  ...
  /// ZIP filename prefix from [AppInfo.appName] only (spaces → underscores, strip path chars).
