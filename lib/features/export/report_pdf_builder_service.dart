...
class ReportPdfBuilderService {
  ...
  pw.Widget _fieldSummaryHeaderBar(
      StandaloneReportData data, pw.ImageProvider? logo) {
    final t = data.trial;
  ...
  pw.Widget _fieldSummaryStatusStrip(
    ...
    final sessionsLine = 'Sessions: $completed of $total completed';
  ...
  List<pw.Widget> _fieldSummaryResultsBlock(StandaloneReportData data) {
    ...
    for (final entry in byAssessment.entries) {
      ...
      final sortedCodes = means.keys.toList()
        ..sort(
            (a, b) => _safeTreatmentLabel(a).compareTo(_safeTreatmentLabel(b)));
  ...
  List<pw.Widget> _buildSiteDescriptionSection(TrialReportSummary t) {
    ...
    if (t.organicMatterPct != null) {
      conditionRows.add(_kvRow('Organic Matter', '${t.organicMatterPct}%'));
    }
    if (t.soilPh != null) {
    ...
    widgets.add(
      pw.Padding(
        ...
        child: pw.Text(
          ...
          style: const pw.TextStyle(
              fontSize: _kFontSizeBody, color: _kTextSecondary),
        ),
      ),
    );
    widgets.add(
    ...
    return pw.Padding(
      ...
      child: pw.Text(
        ...
        style: pw.TextStyle(
          ...
          fontStyle: isApplied ? pw.FontStyle.normal : pw.FontStyle.italic,
        ),
      ),
    );
  }
  ...
  pw.Widget _buildResultsTable(List<RatingResultRow> ratings) {
    ...
    for (final entry in byAssessment.entries) {
      ...
      if (byTreatment.isNotEmpty) {
        ...
        if (resultDirection == AssessmentResultDirection.higherBetter) {
          bestLabel =
              'Best Treatment: ${highest.key} (mean ${_formatMean(highest.value)})';
          worstLabel =
              'Lowest Treatment: ${lowest.key} (mean ${_formatMean(lowest.value)})';
        } else if (resultDirection == AssessmentResultDirection.lowerBetter) {
          bestLabel =
              'Best Treatment: ${lowest.key} (mean ${_formatMean(lowest.value)})';
          worstLabel =
              'Highest Treatment: ${highest.key} (mean ${_formatMean(highest.value)})';
        } else {
          bestLabel =
              'Highest treatment mean: ${highest.key} (${_formatMean(highest.value)})';
          worstLabel =
              'Lowest treatment mean: ${lowest.key} (${_formatMean(lowest.value)})';
        }
      ...
      widgets.add(pw.Table(
        ...
        children: [
          ...
          ...rows.map((r) => pw.TableRow(
                children: [
                  _tableCell(r.plotId),
                  _tableCell(r.rep.toString(), rightAlign: true),
