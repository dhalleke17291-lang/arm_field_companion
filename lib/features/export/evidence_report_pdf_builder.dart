...
class EvidenceReportPdfBuilder {
  ...
  static String raterDriftLabelForTesting(EvidenceRater rater) =>
      _raterDriftLabel(rater);

  static String raterDriftNoteForTesting(EvidenceRater rater) =>
      _raterDriftNote(rater);

  static List<String> rawDataHeadersForTesting() => _rawDataHeaders;

  static List<List<String>> rawDataRowsForTesting(
    List<EvidenceRawDataRow> rows,
  ) =>
      _rawDataRows(rows);

  static String rawDataTruncationWarningForTesting(EvidenceReportData data) =>
      _rawDataTruncationWarning(data);
  ...
  Future<Uint8List> build(EvidenceReportData data) async {
    ...
    pdf.addPage(pw.MultiPage(
      ...
      footer: (ctx) => _pageFooter(ctx),
      build: (ctx) => [
        _sectionTitle('14. Raw Data Appendix'),
        _rawDataAppendixSection(data),
      ],
    ));
  ...
  pw.Widget _rawDataAppendixSection(EvidenceReportData data) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
          'Current rating values per plot, session, and assessment. Amendments and corrections are documented in §9.',
          style: const pw.TextStyle(fontSize: 8, color: _textSecondary),
        ),
        if (data.rawDataTruncated) ...[
          pw.SizedBox(height: 6),
          pw.Container(
            width: double.infinity,
            padding: const pw.EdgeInsets.all(6),
            decoration: pw.BoxDecoration(
              border: pw.Border.all(color: _warningAmber, width: 0.7),
              color: const PdfColor.fromInt(0xFFFFF3E0),
            ),
            child: pw.Text(
              _rawDataTruncationWarning(data),
              style: const pw.TextStyle(fontSize: 8, color: _warningAmber),
            ),
          ),
        ],
        pw.SizedBox(height: 6),
        if (data.rawDataRows.isEmpty)
          pw.Text(
            'No rating records found for this trial.',
            style: const pw.TextStyle(fontSize: 8, color: _textSecondary),
          )
        else
          pw.TableHelper.fromTextArray(
            headers: _rawDataHeaders,
            data: _rawDataRows(data.rawDataRows),
            headerStyle: _tableHeaderStyle(),
            headerDecoration: const pw.BoxDecoration(color: _primary),
            cellStyle: const pw.TextStyle(fontSize: 7),
            cellAlignment: pw.Alignment.center,
  ...
  static String _rawDataTruncationWarning(EvidenceReportData data) {
    return 'This trial contains ${data.rawDataTotalCount} rating records. This appendix shows the first 2,000 rows. Export Raw CSV Data for the complete record.';
  }

  static List<List<String>> _rawDataRows(List<EvidenceRawDataRow> rows) {
    return rows
        .map((row) => [
              row.sessionName,
              row.plotCode,
              ...
              row.treatmentCode,
              row.assessmentName,
              row.ratingValue?.toStringAsFixed(2) ?? '—',
              row.dat != null ? 'DAT+${row.dat}' : '—',
              row.raterName ?? '—',
            ])
        .toList();
  }
  ...
  pw.Widget _deviceRaterSection(EvidenceDataIntegrity di) {
    return pw.Column(
      ...
      children: [
        ...
        pw.SizedBox(height: 4),
        pw.TableHelper.fromTextArray(
          headers: ['Rater', 'Ratings', 'Sessions', 'Drift', 'Note'],
        ...
        pw.SizedBox(height: 4),
        pw.Text(
          'Drift signals are MEDIUM confidence - observed pattern, not a confirmed measurement. Raised automatically at session close when rating pattern variability exceeds threshold.',
          style: const pw.TextStyle(fontSize: 7, color: _textSecondary),
        ),
      ],
    );
  ...
  static String _raterDriftLabel(EvidenceRater rater) {
    return rater.raterDriftDetected
        ? (rater.driftSeverity ?? 'review')
        : 'None detected';
  }

  static String _raterDriftNote(EvidenceRater rater) {
    if (!rater.raterDriftDetected) return '—';
    final note = rater.driftConsequence ?? 'Rater drift detected.';
    if (note.length <= 80) return note;
    return '${note.substring(0, 77)}...';
  }
