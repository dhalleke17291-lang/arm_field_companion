...
class ExportTrialUseCase {
  ...
  /// Human-readable entry point inside the ARM handoff ZIP (alongside CSV tables).
  String _trialHandoffReadmeText({
    required String trialName,
    required String exportedAtLabel,
    required bool includesStatisticsCsv,
    required bool includesWeatherCsv,
  }) {
    final buf = StringBuffer();
    buf.writeln('${AppInfo.appName} — Trial export bundle (ZIP)');
    buf.writeln('App version: ${AppInfo.appVersion}');
    buf.writeln('Trial: $trialName');
    buf.writeln('Exported: $exportedAtLabel');
    buf.writeln();
    buf.writeln(
        'This archive contains tab-separated research tables and optional photos.');
    buf.writeln('Start with import_guide.csv for context and validation notes.');
    buf.writeln();
    buf.writeln('Key files:');
    buf.writeln('- import_guide.csv — how to read this bundle');
    buf.writeln('- data_dictionary.csv — column definitions');
    buf.writeln('- validation_report.csv — export-time checks');
    buf.writeln('- arm_mapping.csv — ARM column mapping reference');
    buf.writeln();
    buf.writeln('Data tables:');
    buf.writeln('- observations.csv, observations_arm_transfer.csv');
    buf.writeln('- treatments.csv, plot_assignments.csv');
    buf.writeln('- sessions.csv, applications.csv, seeding.csv, notes.csv');
    if (includesStatisticsCsv) {
      buf.writeln('- statistics.csv — summary statistics (standalone trials)');
    }
    if (includesWeatherCsv) {
      buf.writeln();
      buf.writeln('Weather:');
      buf.writeln('- weather.csv');
    }
    buf.writeln();
    buf.writeln('Photos (if present):');
    buf.writeln('- photos/ — image files');
    buf.writeln('- photos/photos_manifest.csv — photo index');
    buf.writeln(
        '- photos/photos_missing.csv — files that could not be exported (if any)');
    buf.writeln();
    buf.writeln(
        'For a separate JSON machine-readable export, use the in-app JSON export when available.');
    buf.writeln(
        'Generated by ${AppInfo.appName}. Preserve CSV headers if downstream tools expect them.');
    return buf.toString();
  }
  ...
  Future<File> _buildArmHandoffPackage(
    ...
    final exportedReadable = DateFormat('yyyy-MM-dd HH:mm:ss').format(now);

    final readmeText = _trialHandoffReadmeText(
