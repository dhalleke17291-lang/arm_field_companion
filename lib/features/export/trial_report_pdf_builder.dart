...
class TrialReportPdfBuilder {
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        ...
        _treatmentSummaryStats(
            ...
            isArmLinked: trial.workspaceType != 'standalone'),
        pw.SizedBox(height: 16),
  ...
  pw.Widget _treatmentSummaryStats(
    ...
    // Identify check treatments.
    final checks =
        treatmentById.values.where(isCheckTreatment).toList();
    final String checkCase;
    if (checks.isEmpty) {
      checkCase = 'none';
    ...
    // Compute arithmetic check mean per assessment (single check only).
    final checkMeanByAssessment = <int, double>{};
    if (checkCase == 'single') {
      final checkTrtId = checks.first.id;
      for (final a in assessments) {
        final vals = <double>[];
        for (final s in sessions) {
          for (final p in dataPlots) {
            if ((plotTreatment[p.id] ?? p.treatmentId) != checkTrtId) continue;
    ...
    for (final tid in trtIds) {
      ...
      final isCheckTrt = isCheckTreatment(trt);
      for (final a in assessments) {
        final values = <double>[];
        for (final s in sessions) {
          for (final p in dataPlots) {
        ...
        final pctOfCheck = pctOfCheckCell(
          isCheckTrt: isCheckTrt,
          checkCase: checkCase,
          isArmLinked: isArmLinked,
          checkMean: checkMeanByAssessment[a.id],
          treatmentMean: mean,
        );
    ...
    if (checkCase == 'none') {
      footnote =
          '† No check treatment identified. % of Check not computed.';
    } else if (checkCase == 'multiple') {
      footnote =
          '† Multiple check treatments identified. % of Check requires manual calculation.';
    } else if (isArmLinked) {
      footnote =
          '† % of Check not shown. See ARM AOV report for LSMeans-based values.';
    } else {
      footnote =
          '† % of Check computed from arithmetic treatment means. ARM AOV LSMeans-based values may differ.';
    }
  ...
  /// Computes the % of Check cell value for a single treatment × assessment row.
  ///
  /// Returns the formatted string that should appear in the "% of Check" column.
  /// All four cases (none, multiple, single ARM-linked, single standalone) plus
  /// the check-treatment-row case are handled here so they can be unit-tested
  /// without parsing PDF bytes.
  @visibleForTesting
  static String pctOfCheckCell({
    required bool isCheckTrt,
    required String checkCase,
    required bool isArmLinked,
    required double? checkMean,
    required double treatmentMean,
  }) {
    if (isCheckTrt) return '—';
    if (checkCase == 'none' || checkCase == 'multiple') return '—';
    if (isArmLinked) return '';
    // Single check, standalone.
    if (checkMean == null || checkMean.abs() < 0.0001) return '—';
    return '${((treatmentMean / checkMean) * 100).toStringAsFixed(1)}%';
  }
